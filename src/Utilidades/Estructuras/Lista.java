...
public final class Lista {
    ...
    public String imprimirTexto(boolean numerada) {
        String[] opciones = elementos.toArray(new String[0]);
        if (numerada) {
            return ListaNumerada.imprimirListaNumeradaTexto(opciones);
        } else {
            return ListaConVinetas.imprimirListaConVinetasTexto(opciones);
        }
    }
    public boolean duplicado (Object dato) {
        return this.elementos.contains(String.valueOf(dato));
    }
}