...
public class ImpresionLista extends Impresora {
    ...
    public static String imprimirListaTexto(String[] opciones, boolean conNumeros) {
        if (opciones == null || opciones.length == 0) {
            Impresora.imprimirAviso("Contenido Faltante", "No hay opciones disponibles.");
            return null;
        }
        ...
        for (int i = 0; i < opciones.length; i++) {
            String formato = conNumeros ? "%d. %s" : "- %s";
            if (conNumeros) {
                listaTexto.append(String.format(formato, (i + 1), opciones[i]));
            } else {
                listaTexto.append(String.format(formato, opciones[i]));
            }
        }
