...
public final class Menu extends MenuBase implements AccionMenu {
    ...
    public void mostrar() {
        ...
        Lista menuLista = new Lista();
        for (String opcion : opciones)
            menuLista.agregar(opcion);
        menuLista.agregar("Salir");
        menuLista.imprimir(true);
    }
    /**
    ...
    public void ejecutar() {
        ...
        do {
            ...
            if (eleccion <= opciones.size())
                acciones.get(eleccion - 1).ejecutar();
            else if (eleccion != opciones.size() + 1)
                imprimirError("Menu", "Opcion no valida. Intenta de nuevo.");
